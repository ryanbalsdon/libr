...
struct R_MutableString {
	...
	R_Data* array;         //Array of characters
};
...
const char* R_FUNCTION_ATTRIBUTES R_MutableString_getString(R_MutableString* self) {
	...
	self->cstring = (char*)os_malloc((R_Data_size(self->array)+1)*sizeof(char));
	os_memcpy(self->cstring, R_Data_bytes(self->array), R_Data_size(self->array));
	self->cstring[R_Data_size(self->array)] = '\0';
	return self->cstring;
}
...
R_MutableString* R_FUNCTION_ATTRIBUTES R_MutableString_appendBytes(R_MutableString* self, const char* bytes, size_t byteCount) {
	...
	if (R_Data_appendCArray(self->array, (const uint8_t*)bytes, byteCount) == NULL) return NULL;
	return self;
}
...
R_MutableString* R_FUNCTION_ATTRIBUTES R_MutableString_appendArray(R_MutableString* self, const R_Data* array) {
	...
	if (R_Data_appendArray(self->array, array) == NULL) return NULL;
	return self;
}

size_t R_FUNCTION_ATTRIBUTES R_MutableString_length(const R_MutableString* self) {
	if (self == NULL) return 0;
	return R_Data_size(self->array);
}

R_MutableString* R_FUNCTION_ATTRIBUTES R_MutableString_appendArrayAsHex(R_MutableString* self, const R_Data* array) {
	if (self == NULL || array == NULL) return NULL;
	for (int i=0; i<R_Data_size(array); i++) {
		char hexDigitF0 = (R_Data_byte(array,i) & 0xF0) >> 4;
		char hexDigit0F = (R_Data_byte(array,i) & 0x0F);
		...
		R_Data_push(self->array, hexDigitF0);
		R_Data_push(self->array, hexDigit0F);
	}
...
R_MutableString* R_FUNCTION_ATTRIBUTES R_MutableString_getSubstring(R_MutableString* self, R_MutableString* output, size_t startingIndex, size_t length) {
	...
	R_Data* to_copy = R_Type_Copy(self->array);
	R_MutableString_reset(output);
	R_Data_moveSubArray(output->array, to_copy, startingIndex, length);
...
R_MutableString* R_FUNCTION_ATTRIBUTES R_MutableString_moveSubstring(R_MutableString* self, R_MutableString* output, size_t startingIndex, size_t length) {
	if (self == NULL || output == NULL) return NULL;
	R_MutableString_reset(output);
	R_Data_moveSubArray(output->array, self->array, startingIndex, length);
	return output;
}
...
bool R_FUNCTION_ATTRIBUTES R_MutableString_isSame(R_MutableString* self, R_MutableString* comparor) {
	...
	if (R_Data_compare(self->array, comparor->array) != 0) return false;
	return true;
}
...
bool R_FUNCTION_ATTRIBUTES R_MutableString_compare(const R_MutableString* self, const char* comparor) {
	...
	return (R_Data_compareWithCArray(self->array, (uint8_t*)comparor, os_strlen(comparor)) == 0);
}
...
char R_FUNCTION_ATTRIBUTES R_MutableString_character(R_MutableString* self, size_t index) {
	...
	return (char)R_Data_byte(self->array, index);
}

const R_Data* R_FUNCTION_ATTRIBUTES R_MutableString_bytes(R_MutableString* self) {
	if (R_Type_IsNotOf(self, R_MutableString)) return NULL;
	return self->array;
}
